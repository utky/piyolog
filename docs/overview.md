...
2. **EVENTS**: `母乳合計` 行に到達するまで。
   - イベント開始行判定: 行頭 `^\d{1,2}:\d{2}\s+`(時刻の直後は半角スペース区切り。実データ調査で確認。基本3スペースだが1スペースの例外行も少数ある。当初タブ想定だったが実データにタブは存在しなかった)。
...
- **数値/量型**: 体温, のみもの(ml), ミルク(ml), 搾乳/搾母乳(ml), 体重(g), 身長(cm), 頭囲(cm)
...
上記に加えて、種別欄に体調や出来事を直接自由記述した**単発イベント(長尾)**が少数観測された(例: 体温値をそのまま種別名にする、特記事項を種別名として記録する 等)。頻度が極めて低く(全期間で1〜2件)、個人の体調に関する詳細情報を含みうるため、内容そのものはドキュメント化しない。方針P(未知種別は警告で検知)の対象として扱う想定。

※ 全期間棚卸し完了(2026-07-03実施)。和集合は `docs/known_event_types.md` で確定。
...
- [x] Drive 上のファイルを子供別サブフォルダへ移動(完了)。
