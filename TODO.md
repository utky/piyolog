...
- [x] `piyolog_staging` / `piyolog_intermediate` / `piyolog_marts` データセットを Terraform (`tf/modules/bigquery/`) で事前 provision する (dbt が実体テーブル/ビューを管理するため `delete_contents_on_destroy = true`。raw層とは異なり destroy 時のコンテンツ保護は行わない)
